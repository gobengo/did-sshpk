...
await test('SshpkSigner produces same signature as webcrypto api', async () => {
    ...
    if (!('privateKey' in keyPairA))
        throw new Error("expected keyPairA to have privateKey");
    ...
    const imported = await crypto.subtle.importKey('pkcs8', keyPairAExportedToPkcs8, 'Ed25519', true, ['sign']);
